...
export function CardPattern({ mouseX, mouseY, randomString }: any) {
  ...
  return (
    <div className="pointer-events-none">
      <div className="group-hover/card:opacity-50 absolute inset-0  rounded-2xl [mask-image:linear-gradient(white,transparent)]"></div>
      <motion.div
        className="group-hover/card:opacity-100 absolute inset-0 rounded-2xl bg-gradient-to-r from-green-500 to-blue-700  opacity-0 backdrop-blur-xl transition duration-500"
        style={style}
      />
      <motion.div
        className="group-hover/card:opacity-100 absolute inset-0 rounded-2xl opacity-0  mix-blend-overlay"
        style={style}
      >
